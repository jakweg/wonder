...
export const newPipeline = <ShaderGlobals>(
  ...
  return {
    ...
    async useContext(gl: WebGL2RenderingContext): Promise<void> {
      ...
      let i = 0
      for (const e of mappedElements) e.shader = doneShaders[i++]
    ...
    useGame(game: GameState, scheduler: RenderHelperWorkScheduler, visibility: ChunkVisibilityIndex) {
      ...
      lastGame = game
      lastRebuildTick = -1
      for (const e of mappedElements) e.world = e.element.createWorld({ game, scheduler, visibility }, e.world)
    },
    bindGpuWithGameIfCan() {
      if (allocator === null || lastGame === null || !isDoneWithShaders) return

      for (const e of mappedElements) e.bound = e.element.bindWorldData(allocator, e.shader, e.world, e.bound)
    },
    updateWorldIfNeeded() {
      const thisTick = lastGame?.currentTick ?? -1
      if (lastRebuildTick === thisTick) return
      lastRebuildTick = thisTick

      for (const e of mappedElements) e.element.updateWorld(e.shader, e.world, e.bound)
    },
    prepareRender() {
      for (const e of mappedElements) e.element.prepareRender(e.shader, e.world, e.bound)
    },
    doGpuUploads() {
      for (const e of mappedElements) e.element.uploadToGpu(e.shader, e.world, e.bound)
    },
    draw(ctx: RenderContext) {
      for (const e of mappedElements) e.element.draw(ctx, e.shader, e.world, e.bound)
    },
    drawForMousePicker(ctx: RenderContext) {
      for (const e of mappedElements) e.element.drawForMousePicker(ctx, e.shader, e.world, e.bound)
    },
    ...
    cleanUp() {
      ...
      for (const e of mappedElements) e.bound = e.world = e.shader = null
    },
  }
}
