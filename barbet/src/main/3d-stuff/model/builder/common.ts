...
float progress = smoothstep(0.0, 1.0, smoothstep(0.0, ${typeof duration === 'number' ? duration.toFixed(1) : duration}, diff));
float nextRotation = current - float(((a_entityRotation & ${Direction.MaskRotateCounter}U) >> ${Direction.RotateCounterBitShift}U) * 8U);
