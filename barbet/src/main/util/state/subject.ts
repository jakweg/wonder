...
export const reduce = <T, U>(sources: ReadonlyArray<Subject<T>>, transform: (value: T, accumulator: U) => U, start: U): Subject<U> => {
    let cancel: any[] = []
    let values: any[] = []
...
export const intervalProducer = <T>(value: () => T, interval: number): Subject<T> => {
    ...
    const schedule = (set: Setter<T | undefined>) => cancelTimeout = setTimeout(() => {
