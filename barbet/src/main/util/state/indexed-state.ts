...
export default class IndexedState<T> {
  ...
  private constructor(private readonly values: any[], private readonly fieldObservers: ((value: any) => void)[][]) {}
  ...
  public edit(): Batch<T> {
    ...
    return {
      commit() {
        ...
        for (const [key, value] of pendingChanges) anyChanged = self.singleChange(key, value) || anyChanged
  ...
  public observe<K extends keyof T>(key: K, callback: (value: T[K]) => void, initialCall: boolean = true): () => void {
    let listeners = this.fieldObservers[key as any]
    if (listeners === undefined) this.fieldObservers[key as any] = listeners = []
    ...
    if (initialCall) callback(this.values[key as any])
  ...
  public observeEverything(callback: (value: any) => void, initialCall: boolean = true): () => void {
    const newCallback = (value: any) => {
      callback(value)
    }
    this.everythingObservers['push'](newCallback)

    if (initialCall) callback(this.pass())

    return () => {
      const index = this.everythingObservers!['indexOf'](newCallback)
      if (index >= 0) this.everythingObservers!['splice'](index, 1)
    }
  }
