export default class State<T> {
  ...
  private constructor(private current: T) {}
  ...
  public update(update: Partial<T>) {
    ...
    for (const [key, newValue] of Object.entries(update)) {
      ...
      if (oldValue !== newValue) {
        ...
        if (listeners) listenersToCall.push(...listeners.map(l => () => l(newValue)))
      }
    }
  ...
  public observe<K extends keyof T>(key: K, callback: (value: T[K]) => void, initialCall: boolean = true): () => void {
    ...
    if (initialCall) newCallback(value)
  ...
  public observeEverything(callback: (snapshot: T) => void, initialCall: boolean = true): () => void {
    const newCallback = (value: Parameters<typeof callback>[0]) => callback(value)

    this.everythingListeners.push(newCallback)
    if (initialCall) newCallback(this.current)
    return () => {
      const index = this.everythingListeners.indexOf(newCallback) ?? -1
      if (index >= 0) this.everythingListeners.splice(index, 1)
    }
  }
}
