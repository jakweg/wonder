...
export const getSavesList = async (): Promise<string[]> => {
  ...
  const data = (await promiseWrapRequest(
    transaction.objectStore(Constants.STORE_MAIN_DATA).getAllKeys(null, 1),
  )) as string[]
...
export const readSaveData = async (id: string): Promise<any> => {
  ...
  const transaction = db.transaction([Constants.STORE_MAIN_DATA as string], 'readonly')
  const data = (await promiseWrapRequest(
    transaction.objectStore(Constants.STORE_MAIN_DATA)['get'](IDBKeyRange['only'](id)),
  )) as any
...
export const putSaveData = async (id: string, data: any): Promise<void> => {
  ...
  await promiseWrapRequest(transaction.objectStore(Constants.STORE_MAIN_DATA)['put'](objectToPut))
...
export const deleteAllSaves = async (): Promise<void> => {
  ...
  await Promise['all'](
    data['map'](id => promiseWrapRequest(transaction.objectStore(Constants.STORE_MAIN_DATA)['delete'](id))),
  )
