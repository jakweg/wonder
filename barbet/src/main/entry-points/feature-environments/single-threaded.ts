...
export const bind = (args: ConnectArguments): EnvironmentConnection => {
  ...
  return {
    name: 'zero',
    async createNewGame(gameArgs, renderArgs): Promise<CreateGameResult> {
      ...
      session = await createRenderingSession({
        ...
        passedGame: game.passForRenderer(),
        updater: updaterInstance.pass(),
    ...
    async saveGame(saveArgs: SaveGameArguments): Promise<SaveGameResult<any>> {
