...
const createVertexesAndElements = (size: number, scale: number) => {
  if (size !== (size | 0) || size < 3 || size > 100) throw new Error('Invalid size')
...
function extracted(
  ...
  for (let i = 0, s = vertexes.length; i < s; ) {
    ...
    vertexDataToAppend.push(vx, vy, vz, vertexes[i++]!, vertexes[i++]!, vertexes[i++]!)
  }
...
export const appendToMesh = (
  ...
  if (amount > 0) extracted(vertexDataToAppend, elementsDataToAppend, x, y, z, 1)
  if (amount > 1) extracted(vertexDataToAppend, elementsDataToAppend, x - 0.1, y, z + 0.5, 0.5)
  if (amount > 2) extracted(vertexDataToAppend, elementsDataToAppend, x + 0.1, y, z, 0.4)
  if (amount > 3) extracted(vertexDataToAppend, elementsDataToAppend, x - 0.1, y, z, 0.3)
  if (amount > 4) extracted(vertexDataToAppend, elementsDataToAppend, x + 0.5, y, z + 0.4, 0.6)
}
