...
class DelayedComputerImpl implements DelayedComputer {
	...
	public serialize(): unknown {
		...
		for (const item of this.requestsQueue)
			index += encodeRequest(item, requestsArray as unknown as Int32Array, index)

		index = 1
		const resultsArray: number[] = []
		requestsArray.push(this.results.size)
		for (const item of this.results.values())
			index += encodeResult(item, resultsArray as unknown as Int32Array, index)
