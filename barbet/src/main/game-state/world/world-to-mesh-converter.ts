...
export const buildChunkMesh = (world: WorldLike, chunkX: number, chunkZ: number, chunkSize: number): Mesh => {
	...
	const forceAddVertex = (positionIndex: number, x: number, y: number, z: number): number => {
		vertexes.push(x, y, z, NO_COLOR_VALUE, NO_COLOR_VALUE, NO_COLOR_VALUE, NO_FLAGS_VALUE, NO_FLAGS_VALUE)
...
export const buildChunkMeshWithAmbientOcclusion = (world: WorldLike, chunkX: number, chunkZ: number, chunkSize: number): Mesh => {
	const { sizeX, sizeY, sizeZ } = world.size
	const worldData = world.rawBlockData
	...
	// 	return (ambientOcclusionTable[index]!)
	// }
	...
	const computeAmbientOcclusionOld = (x: number, y: number, z: number): number => {
		...
		return (ambientOcclusionTable[index]!) << 16
	}

	// const computeBothAmbientOcclusions = (x: number, y: number, z: number): number => {
	// 	const nx = x + 1
	// 	const ny = y - 1
	// 	const nz = z + 1
	// 	const a = computeAmbientOcclusion(x, y, z)
	// 	const b = computeAmbientOcclusion(nx, y, z)
	// 	const c = computeAmbientOcclusion(nx, y, nz)
	// 	const d = computeAmbientOcclusion(x, y, nz)
	// 	const e = computeAmbientOcclusion(x, ny, z)
	// 	const f = computeAmbientOcclusion(nx, ny, z)
	// 	const g = computeAmbientOcclusion(nx, ny, nz)
	// 	const h = computeAmbientOcclusion(x, ny, nz)

	// 	return ((a & 0b1111) << 16) | ((((a + b + c + d + e + f + g + h) / 8) & 0b1111) << 20)
	// }
	...
	// const putFlatAOFromVertex = (index: number, value: number) => vertexes[index * FLOATS_PER_VERTEX + 6] = (vertexes[index * FLOATS_PER_VERTEX + 6]! & ~(0b1111 << 20)) | ((value & 0b1111) << 20)
	...
	// 	const a = extractAOFromVertex(indices[i * 6 + 0]!)
	// 	const b = extractAOFromVertex(indices[i * 6 + 1]!)
	// 	const c = extractAOFromVertex(indices[i * 6 + 2]!)
	// 	const d = extractAOFromVertex(indices[i * 6 + 3]!)
	// 	const e = extractAOFromVertex(indices[i * 6 + 4]!)
	// 	const f = extractAOFromVertex(indices[i * 6 + 5]!)


	// 	// put flat data into last
	// 	putFlatAOFromVertex(indices[i * 6 + 2]!, ((a + b + c + d + e + f) / 6) | 0)
	// 	putFlatAOFromVertex(indices[i * 6 + 5]!, ((a + b + c + d + e + f) / 6) | 0)
	// }
