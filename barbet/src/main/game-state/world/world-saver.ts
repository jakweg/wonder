...
const saveToUrl = (game: GameState) => {
  ...
  for (let i = 0; i < length; i++) bytes[i] = asString.charCodeAt(i)!
  return URL.createObjectURL(new Blob([bytes]))
}

export const performGameSave = async (game: GameState, saveArgs: SaveGameArguments): Promise<SaveGameResult> => {
