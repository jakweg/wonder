...
): Generator<Readonly<EntityTraitIndicesRecord>> {
...
export const iterateOverAllSelectedEntities = function* (
  container: EntityContainer,
): Generator<Readonly<EntityTraitIndicesRecord>> {
...
export const iterateOverEntitiesHoldingItems = function* (
  container: EntityContainer,
): Generator<Readonly<EntityTraitIndicesRecord>> {
...
export const iterateOverDrawableEntities = function* (
  container: EntityContainer,
): Generator<Readonly<EntityTraitIndicesRecord>> {
...
export const getEntityById_drawableItem = function (
  container: EntityContainer,
  id: number,
): Readonly<EntityTraitIndicesRecord> | null {
  const record = createEmptyTraitRecord()
  const rawData = container.ids.rawData
  for (let i = 0, l = container.ids.size; i < l; i++) {
    const idIndex = i * DataOffsetIds.SIZE
...
export const queryBuildingDataById = (
  container: EntityContainer,
  id: number,
...
export const queryBuildingProgress = (container: EntityContainer, id: number): number | null => {
  const record = createEmptyTraitRecord()
  const rawData = container.ids.rawData
  for (let i = 0, l = container.ids.size; i < l; i++) {
    const idIndex = i * DataOffsetIds.SIZE
