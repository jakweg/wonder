...
export const TimesTable = <T>(
  ...
  const update = () => {
    ...
    if (array['length'] !== 0) {
      ...
      for (let measurementIndex = 0; measurementIndex < MEASUREMENTS_COUNT; ++measurementIndex) {
        ...
        allCells[statsCount * MEASUREMENTS_COUNT + measurementIndex]!['textContent'] = Math.round(
          samples * MEASUREMENTS[measurementIndex]!.interval,
        ).toString()
      }
    }
  ...
  CONFIG.observe('debug/show-info', show => {
    if (show) frameId = requestAnimationFrame(update)
