...
await Promise.all(
  ...
  ).map(entry => fs.rm(`${outputFolder}/${entry}`, { recursive: true }).catch(e => void e)),
)
...
if (isProduction) {
  const excludedProperties = (await fs.readFile('./src/build/exclude-from-mangling.txt', { encoding: 'utf8' }))
    .split('\n')
    .map(e => e.trim())
  ...
  await fs.writeFile('./mappings.txt', JSON.stringify(result.mangleCache, undefined, 3))
  ...
  const getNames = async (currentPath: string) => {
    ...
    for await (const dirEntry of await fs.readdir(currentPath, { withFileTypes: true })) {
  ...
  await Promise.all(files.map(({ name, content }) => fs.writeFile(name, content, { encoding: 'utf8' })))
}

await Promise.all(
  (
    await fs.readdir('src/public')
  ).map(file => fs.cp(`src/public/${file}`, `${outputFolder}/${file}`, { recursive: true, preserveTimestamps: false })),
)
